...
Write-Host "v1.2 - 2024-02-21" -ForegroundColor Cyan
        ...
        foreach($Subid in $SubIds){
            ...
            foreach ($resourceType in $resultAllResourceTypes) {
                ...
                if ($abbreviationObject -and $abbreviationObject.Abbreviation -ne $false) {
                    ...
                    foreach ($kqlFile in $kqlFiles) {
                        ...
                        if ($query -match "development") {
                            Write-Host "IMPORTANT - Query $kqlshort under development - Validate Resources manually" -ForegroundColor Yellow
                            $result = [PSCustomObject]@{
                                recommendationId = $kqlshort.replace('.kql','')
                                name             = "IMPORTANT - Query under development - Validate Resources manually"
                                id               = ""
                                tags             = ""
                                param1           = ""
                                param2           = ""
                            ...
                            $msg = $kqlshort.replace('.kql','')
                            Write-Host "IMPORTANT - Recommendation $msg cannot be validated with ARGs - Validate Resources manually" -ForegroundColor Yellow
                            $result = [PSCustomObject]@{
                                recommendationId = $kqlshort.replace('.kql','')
                                        ...
                                        foreach($result in $TempResult)
                                            {
                                                ...
                                                if($result.name -eq "IMPORTANT - Query under development - Validate Resources manually" -or $result.name -eq "IMPORTANT - Recommendation cannot be validated with ARGs - Validate Resources manually")
                                                    {
                                                        $Global:results += $result
                                                    }
                                            }                                        
                ...
                if($resourceType.type -notin $Global:servicesAbbreviationsArray.Type)
                {
                    ...
                    Write-Host "IMPORTANT - ServiceType $type Not Available in APRL - Validate Resources manually" -ForegroundColor Yellow
                            $result = [PSCustomObject]@{
                                recommendationId = $resourceType.type
                                name             = "IMPORTANT - ServiceType Not Available in APRL - Validate Resources manually if Applicable, if not delete this line"
                                id               = ""
                                tags             = ""
                                param1           = ""
                                param2           = ""
